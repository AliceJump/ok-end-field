...
#   = 410%。每次消耗 1 层猎矢的强化射击额外强制触发一次独立自然爆发 130%
#   （官方文案：不通过附着层数计算）→ 满猎矢口径 = 410% + 130% x 5 = 1060%。
#   猎矢为干员个人池（combat_model STACK_HUNTING_ARROW：上限 4、on_use 消耗），
#   初始 4 层、连携 8 启示转化 4 层，官方标准循环即 4 层全消耗，故采用满猎矢口径；
#   无猎矢保守口径为 540%。表行加总仅 345%（空中普攻只计 1 次）。
# - 提弗洛斯「冰山呼告」（终结技）：强力箭爆炸后进入浮空，之后 5 次空中普攻
#   命中各产生 1 轮箭雨（第 5 轮为强化箭雨）
#   → 完整倍率 = 爆炸 300% + 常规箭雨 75% x 4 + 强化箭雨 200% = 800%。
