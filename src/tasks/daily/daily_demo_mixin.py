...
class DailyDemoFeature:
    ...
    def battle_demo(self):
        ...
        for i in range(max_time):
            ...
            if not self.wait_click_feature(feature=fL.give_gift, time_out=10, raise_if_not_found=False,
                                           click_after_delay=0.5, box=self.box_of_screen(0.944, 0.900, 0.969, 0.941),
                                           after_sleep=2):
                self.mark_task_failure("未找到进入战斗按钮")
                return False
            self.ensure_main()
            self.auto_battle()
            if not self.wait_click_feature(feature=fL.restart_battle, vertical_variance=0.1, time_out=10,
    ...
    def go_to_DemoGraphic(self):
        ...
        demo_enter = None
        for _ in range(4):
            if result := self.wait_feature(feature=[fL.daily_demo_enter, fL.demo_left_time],
                                           box=self.box_of_screen(0.146, 0.094, 0.179, 0.898), time_out=2,
                                           raise_if_not_found=False):
                demo_enter = result
                break
            else:
        ...
        if demo_enter.name == fL.daily_demo_enter:
            ...
            return True
        for _ in range(2):
            if self.wait_click_feature(feature=fL.view_location, time_out=10, raise_if_not_found=False,
                                       click_after_delay=0.5, box=self.box_of_screen(0.5, demo_enter.y / self.height, 1,
                                                                                     demo_enter.y / self.height + (
                                                                                             0.272 - 0.109))):
                break
    ...
    def _demo_click_track_and_transfer(self):
        ...
        if not self.to_near_transfer_point(self.box.bottom_left):
            ...
            return False
        self.ensure_main()
        if not self.align_ocr_or_find_target_to_center(ocr_match_or_feature_name_list=fL.demographic_follow, ocr=False,
                                                       raise_if_fail=False):
            self.mark_task_failure("未找到『进入演算』目标，可能尚未找到正确路线")
            return False
        if not self.navigate_until_target(target=fL.enter_demo, nav=fL.demographic_follow, target_is_ocr=False,
                                          target_vertical_variance=0.05):
    ...
    def enter_page(self):
        """进入关卡选择界面，等待UI稳定。"""
        result = self.wait_feature(feature=fL.enter_demo, time_out=10, raise_if_not_found=False,
                                   box=self.box_of_screen(0.653, 0.574, 0.679, 0.817), settle_time=1)
        if not result:
    ...
    def read_level(self):
        ...
        result = self.wait_feature(feature=fL.level_tip, time_out=10, raise_if_not_found=False,
                                   box=self.box_of_screen(0.120, 0.724, 0.803, 0.750), settle_time=1)
        if not result:
        ...
        level = int(
            (level_x - self.screen_width * start_x) / (self.screen_width * one_level_width))  # 根据等级信息标志的x坐标计算当前等级
