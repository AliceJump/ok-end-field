...
class DailyBattleMixin(MapMixin, ZipLineMixin, BattleMixin, Common):
    ...
    def _switch_team_before_activate_for_gather(self):
        ...
        deploy_results = self.wait_ocr(
            match=[lang_ocr.get_pattern("ocr_text_030"), lang_ocr.get_pattern("ocr_text_017")],
        ...
        if lang_ocr.get_primary_term("ocr_text_030") in deploy_text:
    ...
    def _open_index(self):
        ...
        self.ensure_main()
        self.press_key("f8")
        self.wait_click_ocr(match=lang_ocr.get_pattern("ocr_text_078"), time_out=7, after_sleep=2, box=self.box.top, log=True)
    ...
    def _navigate_via_zip_line(self):
        ...
        if zip_line_str:
            self.wait_click_ocr(match=lang_ocr.get_pattern("board_zipline"), time_out=10, after_sleep=2, recheck_time=1,
    ...
    def _consume_stamina_potions(self):
        ...
        self.wait_ocr(match=lang_ocr.get_pattern("ocr_text_040"), time_out=5, box=self.box.top_left)
        # 支持天和小时单位，按剩余时效升序消耗
        ...
        if not box_list:
            ...
            if not parsed_boxes:
                ...
                self.safe_back(lang_ocr.get_pattern("ocr_text_033"), box=self.box.top_left, time_out=10, ocr_time_out=2)
                return True
            ...
            for _, box, validity_num, validity_unit in parsed_boxes:
                ...
                if not self.wait_click_ocr(match=lang_ocr.get_pattern("ocr_text_072"), box=self.box.bottom_right, after_sleep=2):
                ...
                break
        # 统一出口，保证异常时也能返回主界面
        if not self.safe_back(lang_ocr.get_pattern("ocr_text_033"), box=self.box.top_left, time_out=10, ocr_time_out=2):
            return False
        return True
    ...
    def battle_gather(self):
        ...
        #
        self.navigate_until_target(target_ocr_pattern=lang_ocr.get_regex_pattern("ocr_regex_002"), nav_feature_name=fL.gather_icon_out_map,
                                   time_out=60)
        #
        if self.wait_ocr(match=lang_ocr.get_pattern("ocr_text_050"), box=self.box.bottom_right, time_out=1):
            self.log_info("放弃未领取的奖励")
            self.wait_click_ocr(match=lang_ocr.get_pattern("ocr_text_050"), box=self.box.bottom_right, time_out=5, recheck_time=1,
                                alt=True)
            self.click_confirm()
        if not self._switch_team_before_activate_for_gather():
            ...
            return False
        #
        result = self.wait_ocr(match=lang_ocr.get_pattern("ocr_text_064"), box=self.box.bottom_right, time_out=5)
        if not result:
            self.log_info("没有找到『激发』按钮")
            return False
        self.sleep(1)
        if not self.wait_click_ocr(match=lang_ocr.get_pattern("ocr_text_064"), box=self.box.bottom_right, time_out=5, recheck_time=1,
                                   alt=True):
            self.log_info("没有找到『激发』按钮")
            return False
        # 开战
    ...
    def battle_space(self):
        ...
        self.wait_click_ocr(match=lang_ocr.get_pattern("ocr_text_092"), time_out=5, after_sleep=2, box=self.box.bottom_right, log=True)
        if self.wait_click_ocr(match=lang_ocr.get_pattern("ocr_text_023"), time_out=5, box=self.box.bottom_left, log=True):
    ...
    def _gather_retry_navigate(self):
        ...
        self._navigate_via_zip_line()
        #
        self.navigate_until_target(target_ocr_pattern=lang_ocr.get_pattern("ocr_text_101"), nav_feature_name=fL.gather_icon_out_map,
                                   time_out=60)
    ...
    def to_restart(self):
        ...
        if self.battle_ctx.is_extra_mode:
            ...
            self.wait_click_ocr(match=lang_ocr.get_pattern("ocr_text_043"), time_out=10, after_sleep=2, box=self.box.bottom_right_quarter,
                                log=True, recheck_time=1)
            self.click_confirm()
        else:
            self.wait_click_ocr(match=lang_ocr.get_pattern("ocr_text_099"), box=self.box.bottom_left, log=True, time_out=5,
        ...
        while self.battle_ctx.left_ticket >= self._battle_stage_cost or self.battle_ctx.extra_run_count < self.battle_ctx.extra_run_limit:
            ...
            if self.battle_ctx.is_extra_mode:
                ...
                if self.battle_ctx.left_ticket <= 0:
                    self.wait_click_ocr(match=lang_ocr.get_pattern("ocr_text_076"), box=self.box.bottom_right, log=True, recheck_time=1)
    ...
    def _get_current_stage_reward_tier(self):
        if self.wait_ocr(match=lang_ocr.get_pattern("ocr_text_038"), box=self.box.top, time_out=1, log=True):
            return self.REWARD_TIER_LOW
        if self.wait_ocr(match=lang_ocr.get_pattern("ocr_text_038"), box=self.box.bottom, time_out=1, log=True):
    ...
    def _switch_stage_reward_tier(self):
        ...
        if not self.wait_click_ocr(match=lang_ocr.get_pattern("ocr_text_081"), box=self.box.bottom_right, time_out=6, after_sleep=1):
        ...
        if current_tier == target_tier:
            ...
            candidates = self.wait_ocr(
                match=[lang_ocr.get_pattern("ocr_text_038"), lang_ocr.get_pattern("ocr_text_094")],
                box=target_box,
                time_out=4,
                log=True,
            ) or []
        ...
        if not self.safe_back(match=lang_ocr.get_pattern("ocr_text_092"), box=self.box.bottom_right, time_out=10, ocr_time_out=1):
    ...
    def to_battle(self):
        if self.battle_ctx.category_name != "能量淤积点":
            ...
            end_time = time.time()
            while not self.wait_ocr(match=lang_ocr.get_pattern("ocr_text_045"), time_out=1, box=self.box.top_left, log=True):
                if time.time() - end_time > 300:
                    self.log_info("等待超时，进入协议空间超时")
                    return False
            self.move_keys("w", duration=0.25)
            while not self.wait_ocr(match=lang_ocr.get_pattern("ocr_text_085"), time_out=1, box=self.box.bottom_right, log=True):
                self.move_keys('w', duration=0.25)
            self.press_key("f")
        else:
            self.wait_pop_up(time_out=4)
            end_time = time.time()
            while not self.wait_ocr(match=lang_ocr.get_pattern("ocr_text_043"), time_out=1, box=self.box.top_left, log=True):
    ...
    def get_claim(self):
        ...
        # 等待界面出现“可领取”
        while not self.wait_ocr(match=lang_ocr.get_pattern("ocr_text_025"), box=self.box.top, time_out=1):
            if time.time() - start_time > 60:
                return 0
        ...
        if not self.wait_click_ocr(
                match=lang_ocr.get_pattern("ocr_text_083"),
        ...
        # 点击“领取”，失败则返回0
        self.next_frame()
        if not self.wait_click_ocr(match=lang_ocr.get_pattern("ocr_text_101"), box=self.box.bottom_right, time_out=2, log=True):
