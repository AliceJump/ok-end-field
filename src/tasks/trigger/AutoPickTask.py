    ...
    def run(self):
        ...
        if self.in_combat_world():
            while button_f := self.find_f():
                text_zone = button_f.copy(
                    x_offset=button_f.width * 6,
                    width_offset=button_f.width * 12,
                    y_offset=-button_f.height,
                    height_offset=button_f.height * 12,
                )
                ...
                if any(text in texts[0].name for text in self.white_list):
                    ...
                    self.log_debug(f"pick white_list {texts[0].name}")
                    self.pick()
                    self.sleep(0.2)
                    return
                start = self.active_time()
                icon_zone = button_f.copy(
                    x_offset=button_f.width * 3.3,
                    width_offset=button_f.width * 0.8,
                    y_offset=-button_f.height * 0.2,
                    height_offset=button_f.height * 0.85,
                    name="choice",
                )
                white_percent = 0
                ...
                if white_percent < 0.1:
                    ...
                    self.log_info(f"pick because not gray/white icon {texts} {white_percent}")
...
white_color = {"r": (230, 255), "g": (230, 255), "b": (230, 255)}
