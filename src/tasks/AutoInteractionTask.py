            ...
            if self.find_one(fL.skip_dialog_esc, horizontal_variance=0.05, frame=now):
                ...
                while time.time() - start < 3:
                    ...
                    if confirm:
                        ...
                        return
                    self.next_frame()
            if self.find_one([fL.baker_icon, fL.baker_page_icon], horizontal_variance=0.05, vertical_variance=0.05,
                             frame=now):
                now = self.next_frame()
                ...
                if result := self.find_one(fL.sentence_part_feature, horizontal_variance=0.05, vertical_variance=0.1,
                                           frame=now):
                    self.click(result, after_sleep=0.4)
                if result := self.ocr(match=self.lang.AutoSkipDialogTask.k_92399078,
