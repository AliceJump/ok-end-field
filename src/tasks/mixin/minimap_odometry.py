...
（当前帧 vs 锚帧）用相位相关估计玩家在世界 XZ 平面（地图系、北向上）的位移，
并做长期积分。用于网格导航、送货和交互点定位等需要连续位移反馈的场景。
...
位移提交阈值开启后，只有“锚帧到当前帧”的位移达到阈值才推进积分锚帧；未达到的部分
仍计入对外位置，但不进入长期积分。这样可避免慢走或原地小步走时按采样次数累积漂移。
...
def decompose_body(
    ...
    Args:
        ...
        heading_deg: 罗盘方位角（度，正北 0°、正东 90°、顺时针）。
