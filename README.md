...
- [物品导航](docs/物品导航与实时检测.md)：WebSocket 驱动的物品采集点导航
...
- 演算抽牌：自动进入演算抽牌页，循环抽取直到满足等级变化条件
- 影拓丰碑：自动完成当前所有普通影拓丰碑关卡
- [定时截图](docs/物品导航与实时检测.md)：按间隔自动截图，便于数据采集/样本收集
...
命令行参数由底层 `ok-script` 启动器解析，项目入口 `main.py` 负责传入 `src/config/__init__.py` 中的任务配置。
...
* `-t` 或 `--task`: 启动后自动执行第N个任务。`1` 代表任务列表（文件 [./src/config/__init__.py](./src/config/__init__.py) 列表 `onetime_tasks`）中的第1个（也就是『日常任务』）。
