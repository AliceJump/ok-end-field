...
返回：[开发文档索引](../zh-CN/index.md) / [README](https://github.com/AliceJump/ok-end-field/blob/master/README.md)

本文说明当前版本下，从第一次启动软件开始，账号身份与账号覆盖配置的默认行为；登录切换的执行流程（GUI → 存储 → Mixin → login_flow）见 [切换账户流程分析](切换账户流程分析.md)。日常操作步骤见 [账号配置用户指南](../zh-CN/账号配置用户指南.md)——本文只讲默认行为与实现逻辑，不重复配置步骤。
...
3. 保存账号列表时，账号名与注册表当前 `username` 精确匹配才复用原 `account_id`；历史 `aliases` 不参与这一步。
...
地图同步 `content` 独立存储在 `map_contents[account_id]`，供 [物品导航](../zh-CN/物品导航与实时检测.md) 读取官方地图 WebSocket 凭证。账号页写入 content 时，如果账号尚未注册，会创建注册表 ID。
...
3. 使用 `current_account_id`（或兼容回退账号名）读取当前任务覆盖。多账户循环只做非创建式解析；账号页尚未注册该账号时，运行上下文会临时以账号名作为键，不会自动落盘生成 ID。
...
相关文档：[账号配置用户指南](../zh-CN/账号配置用户指南.md) / [物品导航与实时检测](../zh-CN/物品导航与实时检测.md) / [API 参考](API.md)
