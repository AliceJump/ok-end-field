...
`ZipLineMixin.board_zip_line` 复用自动送货的三阶段查找并点击「登上滑索架」，随后
调用 `zip_line_list_go` 完成距离 OCR、点击与推进，成功后继续下索点后的网格路线。
...
可上下索的航点。由于滑索场景只能从图片/小地图获得水平朝向、拿不到俯仰角，执行前
会用两端滑索的 `(x,z)` 计算世界方位，并通过只转视角、不按 W 的
`aim_view_to_bearing()` 先完成水平粗对准，再由距离 OCR 完成精细对中。
...
uv run --locked python -m unittest tests.TestNavGridIO tests.TestNavGridPlanner tests.TestNavZipLineGraph tests.TestNavRouteFollower tests.TestGridNavigationMixin -v
```
