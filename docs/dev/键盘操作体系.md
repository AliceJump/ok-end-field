...
三个接口都定义在 [`RuntimeMixin`](../../src/core/base_mixin/runtime_mixin.py)，签名为 `(key, down_time=0.02, after_sleep=0, interval=-1)`，并把解析后的键传给 `send_key()`。项目当前没有 `press_game_key()` 公共方法。
