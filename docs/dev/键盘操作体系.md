...
| 文件 | 行号/方法 | 按键 | 原因 |
|------|----------|------|------|
| `src/tasks/mixin/runtime_mixin.py` | `press_key` / `press_industry_key` / `press_combat_key` 方法体 | 动态 | 这三个方法**本身**就是对 `send_key` 的封装，合法调用 |
| `src/tasks/mixin/runtime_mixin.py` | `click_with_alt` 方法 | `alt` | `alt` 为系统修饰键，用于 alt+点击操作，非游戏可配置热键 |
| `src/tasks/mixin/game_flow_mixin.py` | `skip_dialog` 方法 | `esc` | `esc` 为系统通用退出键，非游戏可配置热键 |
...
| `src/tasks/onetime/WarehouseTransferTask.py` | `_ctrl_click` 方法 | `LCONTROL` | `LCONTROL` 为系统修饰键，用于 ctrl+点击多选，非游戏可配置热键 |
...
- [按键函数封装示例](https://github.com/AliceJump/ok-end-field/blob/69676fe978ed243f61e10147839a35f0716c2fef/src/core/BaseEfTask.py#L173)
