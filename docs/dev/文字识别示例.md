...
> 每个示例都是**可直接放入 `src/tasks/onetime/` 并注册运行**的完整任务文件，
> 风格与 `src/tasks/test/` 下的调试任务保持一致。
> 相关示例：[find_feature 操作示例](图像模板匹配示例.md)（模板匹配与点击）。
...
>
> `recheck_time > 0` 会让 `wait_click_ocr` 在找到结果后**等待 `recheck_time` 秒**
>（等待时长即传入的数值），
...
```python
config["onetime_tasks"].append(["src.tasks.onetime.ExampleClickOcr", "ExampleClickOcr"])
```
