...
| `delivery_locations`           | 当前地区可识别的送货起点列表，每项包含 `name` 和世界坐标 `coordinate.x/y/z`。 |
| `delivery_targets_by_location` | 每个地点对应的终点列表，每项包含 `name` 和世界坐标 `coordinate.x/y/z`。 |
...
1. 先填写地区名。
2. 再分别填写送货起点和每个地点对应的终点；每项使用 `{"name": "名称", "coordinate": {"x": null, "y": null, "z": null}}`，坐标可后续手动补充。
3. 为每个地点补上传送点搜索区域。
4. 按需要补充新的接单特征标签。
...
4. 起点、终点坐标均为游戏世界系 `(x, y, z)`；尚未测量时保留 `null`。导航接入应分别通过 `get_delivery_location_coordinate()` / `get_delivery_target_coordinate()` 读取，不要直接依赖 JSON 字段结构。
