...
> 数据已剥离为 JSON：`assets/data/world_map.json`（纯数据），由 `src/data/world_map.py`（薄加载器）读取，保持原导入路径兼容；涉及 FeatureList 枚举的 `higher_order_feature_dict` 仍在加载器中。
...
4. 为新增的地区、据点、商品、关卡等规范名补齐 `assets/lang/world_map.json` 的现有 locale
5. 运行相关任务验证数据是否生效
...
相关文档：[日常任务](../日常任务.md) / [刷体力](../体力本.md) / [仓库物品转移](../仓库物品转移.md)
