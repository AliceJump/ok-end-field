...
`src/config.py` 中当前注册 20 个一次性任务：日常任务、运送委托接取、仓库物品转移、自动送货、刷体力、演算抽牌、影拓丰碑、`启动一次游戏,120s后自动关闭`、实时检测、诊断，以及 10 个开发调试用测试任务（测试、箭头角度实时读取、区域扫描测试、暂停计时验证、蓝点归中测试、等级读取测试、演算入口导航测试、战斗技能位置检测、战斗模板持续检测、鼠标视角旋转系数标定）。当前注册 4 个触发式任务：自动战斗、自动交互、自动拾取、物品导航。其中 `启动一次游戏,120s后自动关闭` 是实际显示名称；`120s` 是进入主页的超时，进入主页后实际默认等待 15 秒再退出。
...
| [自动大世界收菜](../dev/自动大世界收菜.md) | 自动拾取触发任务的白名单/黑名单与检测流程 |
